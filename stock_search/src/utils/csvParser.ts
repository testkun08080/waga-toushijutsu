  ...
  return new Promise((resolve, reject) => {
    Papa.parse(file, {
      ...
      transform: (value: string, header: string) => {
        // 数値フィールドの処理
        const numericFields = [
          '時価総額', 'PBR', '売上高', '営業利益', '営業利益率',
          '当期純利益', '純利益率', 'ROE', '自己資本比率', 'PER(会予)',
          '負債', '流動負債', '流動資産', '総負債', '現金及び現金同等物',
          '投資有価証券', 'ネットキャッシュ（流動資産-負債）', 'ネットキャッシュ比率'
        ];

        if (numericFields.includes(header)) {
          // 単位表記を除去（倍、%、円など）
          let cleanValue = value.replace(/,/g, '').replace(/倍$/, '').replace(/%$/, '').replace(/円$/, '').trim();
