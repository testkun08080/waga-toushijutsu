...
export const getDefaultColumns = (availableColumns: string[]): ColumnConfig[] => {
  const columnDefinitions: Record<string, Omit<ColumnConfig, 'key' | 'visible'>> = {
    ...
    'ネットキャッシュ（流動資産-負債）': { label: 'ネットキャッシュ', category: 'cash' },
                ...
                <div className="space-y-1 ml-6">
                  {cols.map(col => (
                    <label key={col.key} className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className={`text-xs ${col.essential ? 'text-base-content/50' : 'text-base-content/80'}`}>
                        {col.label}
                        {col.essential && <span className="text-xs text-warning ml-1">*</span>}
                      </span>
                    </label>
                  ))}
                </div>
